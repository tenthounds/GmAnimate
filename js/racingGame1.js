...
var racingGame=(function(w){
    ...
    var config={
        runTime:"0.7s", //起跑时间
        runTransform:'translate(-200px)',
        runTransformSprit:'translate(-810px)',
    ...
    _prop.setNameEn=function(name_en){
        nameEn = name_en;
    }
    _prop.getNameEn=function(){
        return nameEn;
    }
    /**
     * 赛车信号灯变换
    ...
    function getWhite_hot_arr(){
        ...
        for(var i=0;i<10;i++){
            ...
            if(name_en=="SMPK10"){
                num=Math.round(-(Math.random()*300+200));
            }else{
                num=Math.round(-(Math.random()*300+10));
            }
     ...
     * 赛车冲刺阶段
     */
    _prop.Sprint=function(number,data,callBack,callBack1){
        ...
        function animate_sprint(i,el){
            ...
            $("#car"+el).addClass(addClassList_before).css({'transform':config.runTransformSprit,'transition':'all '+time+'ms','transition-timing-function': 'linear'});
            ...
            function action_end_handler(event){
                if(i==0){
                    ...
                    if(name_en&&name_en=="SMPK10"){
                        bg.append('<div class="sm-rank-bg"></div>');
                    }
                ...
                if(name_en&&name_en=="SMPK10"){
                    bg.find('.sm-rank-bg').append('<div class="sm-rank sm-rank'+el+'"></div>');
                }else{
                    $("#car"+el).append(rank).find('.rank ').addClass('rank'+j);
                }
    ...
    _prop.ReStart=function(){
        ...
        bg.find('.sm-rank-bg').hide().find('sm-rank').remove();
